...
describe('Accordion.cy.jsx', () => {
  it('Items Accordion Testing', () => {
    cy.mount(<ItemsAccordion items={items}/>)
    //assert we have three items in the accordion
    // [data-test^="accordion-item"]' -> CSS attribute selector, ^= means starts with
    // returns a collection of elements that match the specified selector
    cy.getDataTest('accordion-wrapper').within(() => {
      cy.get('[data-test^="accordion-item"]').should('have.length', 3)
    })
    // accordion appears closed by default
    cy.contains('Your tests will exist in a describe block.').should('not.be.visible')
    // // click on one arrow and the accordion opens
    ...
    // // click arrow again and the accordion closes
    cy.getDataTest('accordion-item-1').within(() => cy.get('[role=button]').click())
    // cy.contains('Your tests will exist in a describe block').should('not.be.visible')
  })
})